...
int main(int argc, char *argv[])
{
    ...
    int max_capture_time = 0;
    int min_capture_time = 1000000;
    ...
    while (true)
    {
        ...
        // Read the next frame
        capture >> frame;
        auto capture_end = chrono::high_resolution_clock::now();
        int capture_duration = chrono::duration_cast<chrono::microseconds>(capture_end - capture_start).count();
        printf("capture_duration: %d us\n", capture_duration);
        if (capture_duration > max_capture_time)
        {
            max_capture_time = capture_duration;
        }
        if (capture_duration < min_capture_time)
        {
            min_capture_time = capture_duration;
        }
    ...
    // printf("Max Capture Time: %d us\n", max_capture_time);
    // printf("Min Capture Time: %d us\n", min_capture_time);
    printf("Average Capture Time: %f us\n", (float)total_capture_time / (float)frame_num);
